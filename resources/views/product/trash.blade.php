...
<script>
    ...
    $(function () {
        ...
        var dt_product_table = $('.datatables-products'),
            statusObj = {
                1: { title: 'Xuất bản', class: 'bg-label-success' },
                2: { title: 'Chờ xét duyệt', class: 'bg-label-warning' },
                3: { title: 'Bản nháp', class: 'bg-label-secondary' },
            };

        // E-commerce Products datatable
        if (dt_product_table.length) {
            var dt_products = dt_product_table.DataTable({
                ajax: { url: "{{route('products.data.table.trash')}}", method: "get", dataType: "json", },
                columns: [
                    {
                        data: 'name', render: function (data, type, full, meta) {
                            var $name = full['name'],
                                $image = full['images'][1]?.url;
                            if ($image) {
                                // For Avatar image
                                var $output =
                ...
                initComplete: function () {
                    ...
                    this.api()
                        ...
                        .every(function () {
                            var column = this;
                            var select = $(
                                '<select id="ProductCategory" class="select2 form-select select2 text-capitalize"><option value="">Thương hiệu</option></select>'
                            )
                    ...
                    //         var column = this;
                    //         var select = $(
                    //             '<select id="ProductStock" class="form-select select2 text-capitalize"><option value=""> Trạng thái </option></select>'
                    //         )
